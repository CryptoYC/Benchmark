...
这正是Holochain在试图完成的事，每个节点都可以拥有自己的真相源，只有当这些需要与其他节点共享时才需要形成共识，因此，人们可以在Holochain的源头中构建自己的环境并构建自己的dApp（或hApp），若非必要，就无需与分布式哈希表（DHT）中的其余部分进行交互。

我认为这是Holochain与Blockchain的一个重要区别。“Holochain是一个不断验证的分布式哈希表（DHT），其中的每个节点都会对数据发起自己的签名以执行数据验证，这并非是真正意义上的共识，而是所谓的“区块链式”。

所以这里需要注意到的事每个人都会拥有自己的哈希链，如果我想将资金转移给你，我们都需要在自己的本地副本上进行签名操作，随后再将其分享给外部的节点。

这里的想法就是并非所有的数据都与要与所有人共享，如果A与B之间希望进行一笔转账，只要他们双方同意，C和其他人其实都没有必要知道这笔交易。对于不需要所有方共同参与的系统，比如社交网络、聊天程序、P2P平台、协作文档等而言，这都是一个好主意，着本质上是一个大型的共享Git仓库协作。
...
DHT的主要安全问题是Sybilattack。正如Douceur [5]所描述的那样，它包括创建大量被称为“Sybils”的虚假同伴，并在DHT中以战略方式将其置于其中以控制其中的一部分。 Steiner等人成功地对KAD发起了这次攻击[14]。他们从DHT的一个小区域中的一台计算机中注入了216个Sybils，这样他们就能够收集大部分发布和搜索请求内容的索引内容。他们还实现了更多本地化的攻击攻击，使一些关键词在Sybils上被索引从DHT中消失。 KAD的另一个安全威胁是在同行的路由表中覆盖合法联系人的IP地址[15]。大规模利用的KADID欺骗允许攻击者对DHT进行分区，并以极少的资源进行大规模拒绝服务。但是，这两种攻击在KAD客户端的最新版本中得到了缓解[1]。已经引入了几种保护机制：1）防止洪水侵害降低了爬虫的效率，2）不再可能感染具有相同或非常接近的IP地址（相同/ 24子网）的Sybils的对等路由表，以及3）三路握手防止对等体通过声称相同的KADID覆盖路由表中的另一个IP地址.KAD上的经典Sybil攻击[14]直接在路由表中注入思想变得效率低下。虽然已设置保护规则来保护路由表免受Sybil攻击，即使在最新版本中，查找过程仍然不受保护。实际上，为了控制或干扰索引机制，[2]和[6]采用了一种策略，专注于搜索过程而不是对等的路由表。很少有分布式对等体（具有不同的IP地址）比任何合法对等体更接近目标DHTentry，如图1和表I所示。另外，为了确保插入的对等体被发现并吸引所有服务请求，它们在查找期间合作促进彼此处理。在[2]中，所提出的架构HAMACK允许我们用少量资源（~20个节点）监视，遮蔽或污染存储在KAD中的任何内容，而[6]提出了进行日食攻击的有效策略。

Sybil Attack是在对等网络中看到的一种攻击，其中网络中的节点同时主动运行多个身份并破坏信誉系统中的权限/功能。 此攻击的主要目的是获得网络中的大部分影响，以执行系统中的非法（关于网络中设置的规则和法律）操作。 单个实体（计算机）具有创建和操作多个身份（用户帐户，基于IP地址的帐户）的能力。 对于外部观察者来说，这些多个假身份似乎是真正独特的身份。

DHT的攻击方法
大致归纳为七种。我们假设攻击者是具有破坏性意图的个体，暂时将其称为E;受害者可能是一个或多个人，我们称之为V：
Sybil攻击：很多E的同伴加入了这个班级，而E一个人在这个班级中占主导地位。
在DHT中，只有可以代表节点“身份”的节点号，但是，节点号太薄且易于修改，因此攻击者可以批量生产假身份潜行并控制整个网络。身份是一个难题。在今天的网络协议中，我们只能依靠少数权威机构（如政府或互联网服务提供商）提供身份认证。这显然不是一种分散的，以隐私为导向的方法。
Eclipse Attack：E与V的邻居串联，因此V被隔离，E可以知道所有V条的内容。
由于节点号可以自由更改，因此攻击者可以选择特定的受害者并将攻击节点的数量修改为接近受害者，以便受害者的路由表被攻击者的节点占用。一旦受害者在攻击者的网络中“孤立”，攻击者就可以阻止，监视甚至篡改受害者的查询消息。
流失攻击：E经常在课堂上进出教室，因此V无法找到分数并影响V-message中继的效率。
由于加入/退出DHT几乎不需要资源，攻击者可以频繁地进入和离开网络，导致相邻节点耗尽路由表的更新，导致网络拥塞和离线资源不可用。特别是在Chord缺乏冗余的情况下（路由表仅对应于每个间隔中的一个节点）并且路由更新过程非常麻烦（需要4个步骤），它甚至更能抵抗干扰攻击。
对抗性路由：E是V的随机条带。
在DHT的开放环境中，参与者的行为是不可预测的。攻击者可以将消息中继到攻击者的隔离网络，也可以直接拒绝处理，无论路由表如何。对于没有冗余的Chord，这种攻击是完全相同的。棘手。一个运作良好的开放，分散的系统需要基于参与者的正常行为;一个设计良好，开放，分散的系统必须确保系统运行良好，这意味着它奖励正常行为并惩罚随机行为或限制随机行为。发生了。根据这样的定义，DHT缺乏奖励和惩罚机制以及约束机制，这不是一个精心设计的开放和分散的系统。
污染攻击：E篡改他的分数并使V得分错误。
